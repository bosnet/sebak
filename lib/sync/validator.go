...
func NewBlockValidator(nw network.Network,
	ldb *storage.LevelDBBackend,
	networkID []byte,
	conf common.Config,
	opts ...BlockValidatorOption) *BlockValidator {
	v := &BlockValidator{
		network:              nw,
		storage:              ldb,
		networkID:            networkID,
...
func (v *BlockValidator) validateBlock(ctx context.Context, si *SyncInfo, prevBlk *block.Block) error {
	...
	proposer := si.Block.Proposer
	round := si.Block.Round
	round.BlockHash = prevBlk.Hash
	proposerTx := si.Block.ProposerTransaction
	confirmed := si.Block.Confirmed

	blk := block.NewBlock(proposer, round, proposerTx, txs, confirmed)
...
func (v *BlockValidator) validateTxs(ctx context.Context, si *SyncInfo) error {
	for _, tx := range si.Txs {
		...
		if err := tx.IsWellFormed(v.networkID, v.commonConf); err != nil {
