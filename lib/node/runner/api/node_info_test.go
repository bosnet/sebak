...
func TestAPIGetNodeInfoHandler(t *testing.T) {
	...
	policy := node.NodePolicy{
		...
		BlockTimeDelta:            time.Duration(3) * time.Second,
		TimeoutINIT:               time.Duration(5) * time.Second,
		TimeoutSIGN:               time.Duration(2) * time.Second,
		TimeoutACCEPT:             time.Duration(2) * time.Second,
		TimeoutALLCONFIRM:         time.Duration(120) * time.Second,
		RateLimitRuleAPI:          "200-m",
		RateLimitRuleNode:         "100-s",
		OperationsLimit:           1000,
		TransactionsLimit:         2000,
		OperationsInBallotLimit:   20000,
