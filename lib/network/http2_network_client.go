...
func (c *HTTP2NetworkClient) SendMissingTxs(message sebak.Serializable) (retBody []byte, err error) {
	headers := c.DefaultHeaders()
	headers.Set("Content-Type", "application/json")
	...
	u := c.resolvePath(UrlPathPrefixNode + "/missing_txs")
	...
	defer response.StatusCode == http.StatusOK{
		retBody, err = ioutil.ReadAll(response.Body)
	}
